...
void SuperSeq::createEvent(int position, int noteIndex, bool gate)
{
    ...
    if (readGateBits(events[prevEventPos].data) == HIGH)
    {
        int newPosition = quantizedPosition == 0 ? lengthPPQN - 1 : quantizedPosition - 1;
        uint8_t prevNoteIndex = readDegreeBits(events[prevEventPos].data);
        events[newPosition].data = constructEventData(prevNoteIndex, LOW, true);
    ...
    if (gate == LOW && readStatusBits(events[quantizedPosition].data) && readGateBits(events[quantizedPosition].data) == HIGH)
    {
    ...
    events[newEventPos].data = constructEventData(noteIndex, gate, true);
};
...
void SuperSeq::createChordEvent(int position, uint8_t degrees)
{
    ...
    events[position].data = setStatusBits(true, events[position].data);
};
 ...
 * 
 * @param index the degree index
 * @param gate the state of the gate output (high or low)
 * @param status the status of the event
 * @return uint8_t 
 */
uint8_t SuperSeq::constructEventData(uint8_t index, bool gate, bool status)
{
    uint8_t data = 0x00;
    data = setIndexBits(index, data);
...
uint8_t SuperSeq::setIndexBits(uint8_t index, uint8_t byte)
{
    return byte | index;
}
...
{
    return byte & SEQ_NODE_INDEX_BIT_MASK;
}

uint8_t SuperSeq::setGateBits(bool state, uint8_t byte)
{
    return state ? bitSet(byte, SEQ_NODE_GATE_BIT) : bitClear(byte, SEQ_NODE_GATE_BIT);
}

uint8_t SuperSeq::readGateBits(uint8_t byte)
{
    return bitRead(byte, SEQ_NODE_GATE_BIT);
}

uint8_t SuperSeq::setStatusBits(bool status, uint8_t byte)
{
    return status ? bitSet(byte, SEQ_NODE_STATUS_BIT) : bitClear(byte, SEQ_NODE_STATUS_BIT);
}

uint8_t SuperSeq::readStatusBits(uint8_t byte)
{
    return bitRead(byte, SEQ_NODE_STATUS_BIT);
}
